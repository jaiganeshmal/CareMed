...
const Navbar = () => {
  ...
  const [activeSubcategory, setActiveSubcategory] = useState(null); // for level 3 hover
      ...
      <nav className="bg-white">
        <div className="container mx-auto px-4 py-2 flex justify-between items-center">
          ...
          <ul className="hidden md:flex items-center gap-8 text-gray-700 font-medium">
            <li>
              <Link to="/" className="hover:text-[#0a8ddf] transition">Home</Link>
            </li>
            <li>
              <Link to="/about" className="hover:text-[#0a8ddf] transition">About us</Link>
            </li>
            ...
            <li>
              <Link to="/blogs" className="hover:text-[#0a8ddf] transition">Blog</Link>
            </li>
            <li>
              <Link to="/contact" className="hover:text-[#0a8ddf] transition">Contact</Link>
            </li>
          </ul>
        ...
        {menuOpen && (
          <ul className="md:hidden bg-gray-100 py-4 px-6 flex flex-col gap-4 font-medium text-gray-700">
            <li>
              <Link to="/" onClick={() => setMenuOpen(false)} className="hover:text-[#0a8ddf]">Home</Link>
            </li>
            <li>
              <Link to="/about" onClick={() => setMenuOpen(false)} className="hover:text-[#0a8ddf]">About us</Link>
            </li>
            ...
            <li>
              <div
                ...
                onClick={() => setMobileActiveCat(mobileActiveCat ? null : categories[0])}
              >
                Products
              </div>
              ...
              {mobileActiveCat &&
                categories.map((cat) => (
                  <div key={cat.id} className="pl-4">
                    <div
                      className="py-2 font-semibold text-[#0a8ddf] cursor-pointer"
                      onClick={() =>
                        setMobileActiveCat(
                          mobileActiveCat?.id === cat.id ? null : cat
                        )
                      }
                    >
                      {cat.name}
                    ...
                    {mobileActiveCat?.id === cat.id && (
                      <div className="pl-4 flex flex-col gap-1">
                        {cat.subcategories.map((sub) => (
                          <div key={sub.id}>
                            <div
                              className="text-gray-600 cursor-pointer hover:text-[#0a8ddf]"
                              onClick={() =>
                                setMobileActiveSub(
                                  mobileActiveSub?.id === sub.id ? null : sub
                                )
                              }
                            >
                              {sub.name}
                            </div>
                            {mobileActiveSub?.id === sub.id && (
                              <div className="pl-4 flex flex-col gap-1">
                                {sub.products &&
                                  sub.products.map((prod, idx) => (
                                    <Link
                                      key={idx}
                                      to={prod.pdf ? prod.pdf : "/products"}
                                      target={prod.pdf ? "_blank" : "_self"}
                                      onClick={() => setMenuOpen(false)}
                                      className="text-gray-600 hover:text-[#0a8ddf] text-sm"
                                    >
                                      {prod.title}
                                    </Link>
                                  ))}
            ...
            <li>
              <Link to="/blogs" onClick={() => setMenuOpen(false)} className="hover:text-[#0a8ddf]">Blog</Link>
            </li>
            <li>
              <Link to="/contact" onClick={() => setMenuOpen(false)} className="hover:text-[#0a8ddf]">Contact</Link>
            </li>
          </ul>
        )}
      </nav>
